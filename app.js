...
function openAlbum(i) {
    ...
    main.innerHTML = `
        ...
        <div class="album-header" style="display:flex; gap:20px; align-items:center; margin-bottom:30px;">
            <img src="${currentAlbum.cover}" style="width:160px; border-radius:10px;">
            <div>
                <h1 style="margin:0;">${currentAlbum.title}</h1>
                <p style="color:#fa233b; font-weight:600;">${currentAlbum.artist}</p>
                <div style="color:#888; font-size:12px; margin-top:5px;">${currentAlbum.label} • ${currentAlbum.year}</div>
                <div style="color:#555; font-size:12px;">${currentAlbum.duration}</div>
            </div>
        </div>
    ...
    currentAlbum.songs.forEach((song, index) => {
        const div = document.createElement("div");
        div.style = "padding: 15px; border-bottom: 1px solid #222; cursor: pointer;";
